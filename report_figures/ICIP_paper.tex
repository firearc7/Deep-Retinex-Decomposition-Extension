...
I = R \circ L
\end{equation}
where $\circ$ denotes element-wise multiplication. The encoder uses convolutional layers with ReLU to extract features, while the decoder uses transposed convolutions to reconstruct full-resolution maps. Sigmoid activation ensures outputs are in $[0, 1]$.

\textbf{RelightNet} ($\sim$305K parameters): U-Net architecture with skip connections that enhances illumination~$L$ to produce~$\hat{L}$. Skip connections preserve spatial details. The enhanced image is:
\begin{equation}
\hat{I} = R \circ \hat{L}
\end{equation}
...
\begin{itemize}
    \item \textbf{Unsharp Masking}: Edge enhancement using $I_{sharp} = I + \alpha(I - G_\sigma * I)$ with $\alpha=1.0$, $\sigma=1.0$. Recovers smoothed details.
...
Figure~\ref{fig:training} shows the training convergence. The validation loss reaches its minimum of 0.1640 at epoch 84, representing a 59.8\% reduction from the initial loss. The step decay at epoch 50 enables fine-tuning in the later training phase.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{training_analysis.png}
\caption{Training analysis: Loss convergence showing best validation loss of 0.1640 at epoch 84, with learning rate step decay at epoch 50. Total loss reduction of 59.8\% from initial to best.}
\label{fig:training}
\end{figure}
